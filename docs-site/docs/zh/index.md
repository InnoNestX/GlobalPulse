  ...
  <section class="gp-grid gp-hero" aria-label="GlobalPulse overview">
    ...
    <div class="gp-cell gp-span-7 gp-terminal-wrap">
      <div
        class="gp-term"
        data-gp-terminal
        data-lines='["# 在 Cloudflare Workers 上启动 GlobalPulse","git clone https://github.com/InnoNestX/GlobalPulse.git","cd GlobalPulse","cp .dev.vars.example .dev.vars","cp wrangler.example.jsonc wrangler.jsonc","npm install","npm run deploy","→ Uploaded globalpulse","→ Cron: */5 * * * *","→ Channels: feishu, telegram, wechat, email","✔ 简报管线已上线"]'
      >
        ...
        <div class="gp-term-body" data-gp-terminal-body aria-live="polite"></div>
      </div>
